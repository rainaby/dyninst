...
\end{apient}
\apidesc{Construct a slicer, which will perform forward or backward slicing
starting at Assignment \code{a}. \code{block} and \code{func} represent the
...
\end{apient}
\apidesc{A set of control flow edges that have been visited during slicing.
Users can check this set of edges to determine whether the slicing should be
continued.}	

\begin{apient}
static bool isWidenNode(Node::Ptr n);
\end{apient}
\apidesc{Whether a node is the slice graph is widened}
...
\end{apient}
\apidesc{Create a slice edge from \code{source} to \code{target} and the edge is
labeled with \code{data}.}
...
Predicates(); 
\end{apient}
\apidesc{Construct a default predicate, which will only search for data flow
dependencies and will not clean cache.}

\begin{apient}
bool performCacheClear(); 
\end{apient}
\apidesc{Clear the cache.}

\begin{apient}
void setClearCache(bool b);
\end{apiend}
