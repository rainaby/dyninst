...
static RegisterAST::Ptr archPTX_R11 (new RegisterAST (archPTX::x11));
static RegisterAST::Ptr archPTX_LR  (new RegisterAST (archPTX::x30));
//SP is an independent reg in archPTX
static RegisterAST::Ptr archPTX_SP  (new RegisterAST (archPTX::sp));

bool IA_IAPI::isFrameSetupInsn(Instruction::Ptr i) const
{
    if(i->getOperation().getID() == archPTX_op_mov_add_addsub_imm)
    {
...
bool IA_IAPI::isNop() const
{
    ...
    if(ci->getOperation().getID() == archPTX_op_nop_hint)
	return true;
...
bool IA_IAPI::savesFP() const
{
    ...
    RegisterAST::Ptr returnAddrReg(new RegisterAST(archPTX::x30));

    //stp x29, x30, [sp, imm]!
    if(insn->getOperation().getID() == archPTX_op_stp_gen &&
...
bool IA_IAPI::cleansStack() const
{
    ...
    //ldp x29, x30, [sp], imm
    if(insn->getOperation().getID() == archPTX_op_ldp_gen &&
